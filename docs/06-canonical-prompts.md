...
axf should ship canonical prompts as an onboarding and execution layer for agents.

Prompts are not the architecture.
They are the standardized operating guidance layered on top of the axf contract.
...
Those belong to axf itself.

## Prompt classes to ship

### Framework bootstrap prompt

Used when an agent is helping stand up axf or extend core framework pieces.

### Adapter discovery prompt

Used to inspect a tool and determine whether it is connectable to axf.
...
Used to generate the draft adapter files and tests against axf's declared contract.
...
2. user invokes axf scaffolding/init flow
...
axf succeeds only if different agents using the same docs and prompts converge on roughly the same adapter shape.
